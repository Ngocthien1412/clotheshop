...
/* Make an API request.
   ...
   const apiRequest = async (method, path, body = null) => {
    ...
    try {
      ...
      throw new HTTPError(res.status, "Lỗi khi phân tích phản hồi JSON từ server");
    }
  
    if (!res.ok) {
      throw new HTTPError(res.status, resData && resData.error ? resData.error : "Lỗi không xác định từ server");
    }
